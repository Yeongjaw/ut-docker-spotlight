...
module Spotlight
  ...
  class AdminUsersController < Spotlight::ApplicationController
    ...
    def destroy
      # If remove_all_roles is passed, then remove all user roles. Otherwise,
      # only remove admin role from user.
      user = Spotlight::Engine.user_class.find(params[:id])
      if params[:remove_all_roles]
        if user.roles.delete_all
          flash[:notice] = t('spotlight.admin_users.remove_user_roles.success')
        else
          flash[:error] = t('spotlight.admin_users.remove_user_roles.error')
        end
      else
        # Remove admin role
