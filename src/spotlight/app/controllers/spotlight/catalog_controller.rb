...
module Spotlight
  ...
  class CatalogController < ::CatalogController
    ...
    def edit
      # @response, @document = search_service.fetch params[:id]
      redirect_to admin_exhibit_catalog_path(@exhibit), alert: t(:'helpers.submit.item.not_allowed', model: @exhibit.class.model_name.human.downcase)
    end

    def destroy
      @response, @document = search_service.fetch params[:id]
      resource_id = @document.id.partition('-').last

      # delete items in solr
      solr.delete_by_query("id:#{@document.id}", params: { softCommit: true })

      # delete image from mounted directory, then DB
      Spotlight::Resources::Upload.where("id = '#{resource_id}'").each do |resource|
        Spotlight::FeaturedImage.find(resource.upload_id).destroy
        resource.destroy
      end

      redirect_to admin_exhibit_catalog_path(@exhibit), notice: t(:'helpers.submit.item.destroyed', model: 'item')
    end
