...
module Spotlight
  ...
  class ExhibitsController < Spotlight::ApplicationController
    ...
    def destroy
      ...
      slug_to_delete = @exhibit.attributes["slug"].to_s
      solr.delete_by_query("spotlight_exhibit_slugs_ssim:#{slug_to_delete}", params: { softCommit: true })

      # delete image from mounted directory
      Spotlight::Resources::Upload.where("exhibit_id = '#{@exhibit.id}'").each do |resource|
