  ...
  describe '#to_solr' do
    ...
    describe 'label' do
      ...
      it 'indexes to multiple fields when configured' do
        ...
        allow(Spotlight::Engine.config).to receive(:iiif_creatorContributor_fields).at_least(:once).and_return(%w[creatorContributor_field1 creatorContributor_field2])

        expect(subject.to_solr['title_field1']).to eq 'Test Manifest 1'
        expect(subject.to_solr['title_field2']).to eq 'Test Manifest 1'
        expect(subject.to_solr['creatorContributor_field1']).to eq 'Test Manifest 1'
        expect(subject.to_solr['creatorContributor_field2']).to eq 'Test Manifest 1'
      end
    end
